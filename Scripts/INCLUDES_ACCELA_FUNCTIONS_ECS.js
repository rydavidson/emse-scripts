...
aa.print("in the first method" + fSeqNbr);
        feeSeqNbr = fSeqNbr;
		
aa.print("in the first method" + feeSeqNbr);		
		// var feeSeqNbr = 814761;
	itemCap = capId;
	if (arguments.length == 2) itemCap = arguments[1]; // use cap ID specified in args
	
	//get fee details
	//retrieve a list of invoices by capID
	aa.print("A:" + feeSeqNbr);
	  ...
	  { 	aa.print("B");
		...
		for (iNum in iList)
		  { 	aa.print("C: " + iNum);
			...
			aa.print("flist" + fList);
			for (fNum in fList){
				aa.print("D1: " + fNum + " value: " + fList[fNum].getFeeSeqNbr());
        	  if (fList[fNum].getFeeSeqNbr() == feeSeqNbr)
			    {		aa.print("D");
				  invNbr = iList[iNum].getInvNbr();
				  feeAmount = fList[fNum].getFee();
				  iFound = true;
				  aa.print("Invoice Number Found: " + invNbr);
				  aa.print("Fee Amount: " + feeAmount);
				  break;
			}}
		  }
		  aa.print("iFound: " + iFound);
		  if (!iFound)
			{	aa.print("E");
...
function paymentByTrustAccountHOLD(fSeqNbr) //optional: itemCap
  {
	// function  performs the following:
	// retrieve primary trust account on capId 
	// initiates payment from identified trust account for the ammount of the fee associated with fseqNbr
