...
logDebug("contents of the parameters" + UpdatedRelatedCapIDModelList[0]);
//testGetParm = aa.env.getValue(UpdatedRelatedCapIDModelList);
for (k in UpdatedRelatedCapIDModelList[0]){logDebug("anything: " + k + UpdatedRelatedCapIDModelList[0][k]);}
/*------------------------------------------------------------------------------------------------------/
...
if (feeSeqList.length)
 ...
 // NYELS-52768 Syncing
    logDebug("NYELS-52768 defect syncing");
    var addContactCount = 0;
	ca = getContactArray();
    logDebug("ca: "+ca);
	if (ca) {
   		logDebug("if (ca)");
    	for (x in ca) {
    		if (ca[x] ["contactType"] == "Applicant") addContactCount++;
   			logDebug("ca[x] [contactType]: "+ca[x]["contactType"]);
    	} 
    }

	if (addContactCount > 1) {
	    showMessage = true;
	    cancel = true;
	    comment("You cannot have more than 1 Applicant Contact. Please remove the additional applicant contact.");
	}
